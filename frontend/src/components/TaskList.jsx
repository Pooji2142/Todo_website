...
function TaskList() {
  ...
  const saveModifiedTask = async (modifiedTaskObj) => {
    let res = await axios.put(
      `${apiUrl}/user-api/edit-todo/userid/${currentUser._id}/taskid/${taskBeingEdited._id}`,
      modifiedTaskObj,
      {
  ...
  const setTaskCompleted = async (taskid) => {
    let res = await axios.put(
      `${apiUrl}/user-api/edit-status/userid/${currentUser._id}/taskid/${taskid}`,
      null,
      { withCredentials: true }
    );
    console.log(res);
  ...
  const deleteTask = async (taskid) => {
    let res = await axios.put(`${apiUrl}/user-api/delete-todo/userid/${currentUser._id}/taskid/${taskid}`);
    if (res.status === 200) {
      setCurrentUser(res.data.payload);
    }
  };
        ...
        <div className="mb-3 border border-2 p-3" key={todoObj._id}>
          ...
          <div className="text-start">
            <button
              ...
              disabled={todoObj.status === "completed"} //  disabled if completed
            >
              <FaEdit />
            </button>
          </div>
